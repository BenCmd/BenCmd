...
public class NPCCommands implements Commands {
	...
	public void Npc(String[] args, User user) {
		if (args.length == 0) {
			user.sendMessage(ChatColor.YELLOW + "Proper usage: /npc {bank|bupgrade|blacksmith|remove [id]|despawnall}");
			user.sendMessage(ChatColor.YELLOW + "TIP: Right-click an NPC with a stick to get info about that NPC");
			return;
		}
		if(args[0].equalsIgnoreCase("bank")) {
			plugin.npcs.addNPC(new BankerNPC(plugin, plugin.npcs.nextId(), user.getHandle().getLocation()));
		} else if (args[0].equalsIgnoreCase("bupgrade")) {
			plugin.npcs.addNPC(new BankManagerNPC(plugin, plugin.npcs.nextId(), user.getHandle().getLocation()));
		} else if (args[0].equalsIgnoreCase("blacksmith")) {
			plugin.npcs.addNPC(new BlacksmithNPC(plugin, plugin.npcs.nextId(), user.getHandle().getLocation(), null, null));
			...
			((BlacksmithNPC)npc).addItem(user.getHandle().getItemInHand(), Double.parseDouble(args[2]), user.getHandle());
