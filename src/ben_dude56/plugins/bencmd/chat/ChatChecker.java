...
public class ChatChecker {
	public static boolean checkBlocked(String message, BenCmd plugin) {
		...
		for(String messagePart : splitString) {
			for(String blockedWord : plugin.mainProperties.getString("blockedWords", "").split(",")) {
	...
	public static List<String> splitBy(List<String> remainingChars, String originalMessage, BenCmd plugin) {
		...
		if(remainingChars.size() != 1) {
			...
			for(String toSplit : splitMessage) {
				newSplitMessage.addAll(splitBy(remainingChars, toSplit, plugin));
			}
		}
