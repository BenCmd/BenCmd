...
public class LotCommands implements Commands {
	...
	public boolean onCommand(CommandSender sender, Command command,
		...
		if (commandLabel.equalsIgnoreCase("addguest")) {
			if (args.length==1) {
				plugin.getServer().dispatchCommand(sender, "lot guest +" + args[0]);
			}
				...
				plugin.getServer().dispatchCommand(sender, "lot guest" + args[0] + guest);
			}
			return true;
		}
	...
	public void Lot(String[] args, User user) {
		 ...
		 * LOT INFO
		 * 
		 * This allows all those with the
		 * isLandlord, canCheckLots, or *
		 * permissions to get the info about
		 * a certain lot.
		 */
		if (args[0].equalsIgnoreCase("info")
			...
			if (args.length >= 2) {
				...
				if (!plugin.lots.lotExists(LotID)) {
					user.sendMessage(ChatColor.RED + "Invalid Lot ID."
							+ ChatColor.YELLOW + " Use '/lot info' to get info about a lot you are currently in.");
					return;
				}
			} else {
				if (user.isServer()) {
			...
			if (thisLot != null) {
				user.sendMessage(ChatColor.GRAY + "Lot ID: " + lot
						+ "  Part: " + sub + "   Total parts: " + (thisLot.getSubs().size()));
				user.sendMessage(ChatColor.GRAY + "Owner: "
						+ thisLot.getOwner() + "    Group: "
						+ thisLot.getLotGroup() + "   Guests: "
						+ thisLot.getGuests().size());
				user.sendMessage(ChatColor.GRAY + "Corner 1 - X: " + thisLot.getCorner1().getX()
						+ "  Y: " + thisLot.getCorner1().getBlockY() + "  Z: "
						+ thisLot.getCorner1().getBlockZ());
				user.sendMessage(ChatColor.GRAY + "Corner 2 - X: " + thisLot.getCorner2().getX()
						+ "  Y: " + thisLot.getCorner2().getBlockY() + "  Z: "
		...
		if (args[0].equalsIgnoreCase("guests")
			...
			if (args.length == 1) {
				...
				if (!user.hasPerm("canCheckLots") && !user.hasPerm("isLandlord") && !lot.isOwner(user.getHandle())) {
					user.sendMessage(ChatColor.RED
							+ "You do not have permission to check lots!");
					return;
				}
			...
			if (!args[1].startsWith("+") && !args[1].startsWith("-")) {
				...
				for (String str : args) {
					...
					if (str.startsWith("+")) {
						if (!user.hasPerm("isLandlord") && !lot.isOwner(user.getHandle())) {
							user.sendMessage(ChatColor.RED
						...
						if (Lot.isGuest(str)) {
							...
							user.sendMessage(ChatColor.GREEN + "'" + str
								+ "' is now a guest of lot " + LotID.split(",")[0]);
						}
						...
						if (!Lot.isGuest(str)) {
							user.sendMessage(ChatColor.RED + "'" + str
								+ "' is not a guest of lot " + LotID.split(",")[0]);
						}
						else {
		 ...
		 * LOT SET
		 * 
		 * This creates a new lot using the corners
		 * marked off with the wooden shovel.
		 */
		 ...
		 * LOT ADVANCED SET
		 * 
		 * This allows landlords to lot off
		 * vertically more blocks then they had
		 ...
		 * 
		 * If no lot is specified, then
		 * the lot the player is currently standing
		 * in is deleted.
		 */
		...
		if (args[0].equalsIgnoreCase("setowner")
			...
			if (args.length == 1) {
				user.sendMessage(ChatColor.YELLOW
						+ "Correct usage is: /lot " + args[0] + " <New Owner> <Lot ID>");
				return;
			}
			if (args.length >= 2) {
				String LotID, Owner;
		...
		if (args[0].equalsIgnoreCase("list")) {
			...
			if (args.length == 1) {
				...
				user.sendMessage(ChatColor.GRAY
						+ "Registered lots:  " + size + "   Total Parts:  " + total);
				for (int key : Lot) {
		 ...
		 * LOT GROUP
		 * 
		 * Sets the group that is allowed
		 * to edit the lot.
		 */
		...
		if (args[0].equalsIgnoreCase("extend") || args[0].equalsIgnoreCase("add") || args[0].equalsIgnoreCase("newpart") || args[0].equalsIgnoreCase("ext")) {
			...
			if (cor1 && cor2) {
				...
				plugin.lots.addLot(LotID + "," + SubID, corner1, corner2, owner, group);
				LotID = plugin.lots.getLot(LotID).getLotID();
				int SubSize = plugin.lots.getLot(LotID).getSubs().size();
				user.sendMessage(ChatColor.GREEN + "Lot " + LotID
						+ " was successfully extended to part " + SubID + ".  (" + SubSize + " total parts)");
				return;
			}
			else {
				if (!cor1 && !cor2)
		 ...
		 * to change the y-coordinates of
		 * the extension being created
		 */
