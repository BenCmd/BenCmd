...
public class EntityPermListen extends EntityListener {
	...
	public void onExplosionPrime(ExplosionPrimeEvent event) {
		if(event.getEntity() instanceof TNTPrimed) {
			...
			plugin.getServer().broadcastMessage(ChatColor.RED + "Attempted redstone TNT detonation detected! Nearest player:  " + nearPlayer.getName());
			Logger.getLogger("minecraft").info(logMessage);
			if(plugin.mainProperties.getBoolean("attemptRedstoneTntKick", false)) {
