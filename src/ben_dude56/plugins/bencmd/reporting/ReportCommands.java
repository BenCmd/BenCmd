...
public class ReportCommands implements Commands {
	...
	public void Report(String[] args, User user) {
		...
		PermissionUser reported = PermissionUser.matchUserIgnoreCase(args[0], plugin);
		if(reported == null) {
		...
		plugin.reports.addTicket(new Report(plugin, id, user, reported, Report.ReportStatus.UNREAD, reason, "", 0, new ArrayList<String>()));
		...
		for(Player onlinePlayer : plugin.getServer().getOnlinePlayers()) {
			User onlineUser;
			if((onlineUser = new User(plugin, onlinePlayer)).hasPerm("isTicketAdmin")) {
		...
		if(args[0].equalsIgnoreCase("list")) {
			...
			for(Report ticket : plugin.reports.getReports()) {
				if(ticket.canRead(user)) {
					...
					case READ:
						tickets = true;
						user.sendMessage(ChatColor.YELLOW + ticket.readShorthand());
						break;
					case INVESTIGATING:
						tickets = true;
						user.sendMessage(ChatColor.GREEN + ticket.readShorthand());
						break;
					}
				}
			}
			if(!tickets) {
				user.sendMessage(ChatColor.RED + "You don't have any open tickets!");
			}
			...
			for (Report ticket : plugin.reports.getReports()) {
				if(ticket.getAccused().getName().equalsIgnoreCase(args[1]) || ticket.getSender().getName().equalsIgnoreCase(args[1])) {
					results.add(ticket);
				}
			}
			if (results.isEmpty()) {
				...
				for(Report ticket : results) {
					...
					case INVESTIGATING:
						...
						break;
					case CLOSED:
						user.sendMessage(ChatColor.GRAY + ticket.readShorthand());
						break;
					case LOCKED:
						user.sendMessage(ChatColor.DARK_GRAY + ticket.readShorthand());
						break;
					}
				}
			}
			...
			if(args.length == 1) {
				user.sendMessage(plugin.reports.getTicketById(id).readReport(user.hasPerm("isTicketAdmin")));
				...
				if(user.hasPerm("isTicketAdmin")) {
					...
					if(args.length == 2) {
						...
						log.info(user.getName() + " closed ticket #" + id.toString() + "!");
						...
						log.info(user.getName() + " closed ticket #" + id.toString() + "! Reason: " + reason);
						user.sendMessage(ChatColor.GREEN + "Ticket (ID: " + report.getId() + ") has been successfully closed!");
					}
				} else {
				...
				if(user.hasPerm("isTicketAdmin")) {
					...
					log.info(user.getName() + " re-opened ticket #" + id.toString() + "!");
					...
					if(report.reopenTicket(false)) {
						...
						user.sendMessage(ChatColor.RED + "That ticket has been re-opened too many times!/n" + ChatColor.RED + "Talk to an admin to have it re-opened!");
					}
				} else {
					user.sendMessage(ChatColor.RED + "You cannot edit that ticket!");
				}
				...
				if(user.hasPerm("isTicketAdmin")) {
					...
					log.info(user.getName() + " locked ticket #" + id.toString() + "!");
				} else {
					user.sendMessage(ChatColor.RED + "You must be an admin to do that!");
				}
				...
				if(user.hasPerm("isTicketAdmin")) {
					...
					log.info(user.getName() + " is investigating #" + id.toString() + "!");
				} else {
					user.sendMessage(ChatColor.RED + "You must be an admin to do that!");
				}
				...
				if(user.hasPerm("isTicketAdmin")) {
					...
					log.info(user.getName() + " is no longer investigating #" + id.toString() + "!");
				} else {
					user.sendMessage(ChatColor.RED + "You must be an admin to do that!");
				}
				...
				if(report.canBasicChange(user)) {
					...
					if(report.getStatus() == Report.ReportStatus.CLOSED && !user.hasPerm("isTicketAdmin")) {
						user.sendMessage(ChatColor.RED + "That ticket is closed! Re-open it using /ticket " + report.getId() + " reopen to add new information.");
						return;
					}
					if(report.getStatus() == Report.ReportStatus.INVESTIGATING && !user.hasPerm("isTicketAdmin")) {
					...
					log.info(user.getName() + " has added information to ticket #" + id.toString() + "!");
					if(user.hasPerm("isTicketAdmin")) {
						return;
					}
					for(Player onlinePlayer : plugin.getServer().getOnlinePlayers()) {
