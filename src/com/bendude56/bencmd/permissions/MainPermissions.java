...
public class MainPermissions {
	...
	public MainPermissions() {
		...
		kick = new KickList();
		mp = new MaxPlayers(prop.getInteger("maxPlayers", 10), prop.getInteger("maxReserve", 4), prop.getBoolean("reserveActive", true), prop.getBoolean("indefActive", true));
		loadJail();
	}

	private void loadJail() {
		String str = BenCmd.getMainProperties().getString("jailLocation", "0,0,0,0.0,0.0,world");
	...
	public void setJailWarp(Location l) {
		...
		BenCmd.getMainProperties().setProperty("jailLocation", x + "," + y + "," + z + "," + yaw.toString() + "," + pitch.toString() + "," + world);
