...
public class ItemBW extends BenCmdFile {
	...
	public List<Material> getListed(String group) {
		if (!groupExists(group) && !addGroup(group)) {
			BenCmd.log(Level.SEVERE, group + " was not in plugins/BenCmd/itemsbw.db and couldn't be added due to an unknown error! Returning null...");
			return null;
		}
		List<Material> matList = new ArrayList<Material>();
		try {
			for (String material : getFile().getProperty(group).split("/")[1].split(",")) {
				Material mat;
				try {
					mat = Material.getMaterial(Integer.parseInt(material));
				} catch (NumberFormatException e) {
					BenCmd.log(Level.SEVERE, "Cannot get a number from input: " + material + " in plugins/BenCmd/itemsbw.db (Entry: " + group + ")! Skipping...");
					continue;
				}
				matList.add(mat);
			}
	...
	public boolean canSpawn(Material mat, String group) {
		...
		switch (getSetting(group)) {
			...
			case BWNoRestriction:
				BenCmd.log(Level.WARNING, "Group " + group + " is using a deprecated blacklist/whitelist setting");
				returnValue = true;
				break;
			default:
	...
	public BWSetting getSetting(String group) {
		if (!groupExists(group) && !addGroup(group)) {
			BenCmd.log(Level.SEVERE, group + " was not in plugins/BenCmd/itemsbw.db and couldn't be added due to an unknown error! Returning BWUnknown...");
		...
		if (set.equalsIgnoreCase("b")) {
			...
			BenCmd.log(Level.WARNING, "Cannot get a BWSetting from input: " + set + " in plugins/BenCmd/itemsbw.db (Entry: " + group + ")! Returning BWUnknown...");
			return BWSetting.BWUnknown;
		}
	}
	...
	public boolean addGroup(String group) {
		...
		try {
			getFile().put(group, BenCmd.getMainProperties().getString("defaultItemAction", "b") + "/");
			saveFile();
